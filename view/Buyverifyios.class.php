...
class BuyVaidiOSController extends PageController {
	function process(){
		try{
            ...
            $secret = "";
            if($table0=="cloudlevel")
                $secret = "1868c2da3302475db479c18c5452af51";    // APP固定密钥，在itunes中获取
            else
                $secret = "91ba676daa8146cebe28f9a2a734c8c0";
            //待验证数据 
    ...
    public function check_apple_trans($table,$notify,$receipt_data) {
        ...
        $m = new \woo\mapper\FinalAssembler($table,array("id","origintransid","productid"),$cond);
        $objs = $m->find();
